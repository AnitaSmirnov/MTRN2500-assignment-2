...
MyVehicle::MyVehicle() {
	...
	*/
	...
	//front right wheel
	/* Shape *s = new Cylinder(1.5, 0, 2.65, 1.0, 0.3, 0.0, 1.0, 1.0, 0.3, TRUE, TRUE);
	shapes.push_back(s);
	//front left wheel
	s = new Cylinder(1.5, 0, -2.65, 1.0, 0.3, 0.0, 1.0, 1.0, 0.3, TRUE, TRUE);
	shapes.push_back(s);

	s = new Cylinder(-2, 0, -2.65, 1.0, 0.3, 0.0, 0.3, 1.0, 0.3, TRUE, FALSE);
	shapes.push_back(s);

	s = new Cylinder(-2, 0, 2.65, 1.0, 0.3, 0.0, 0.3, 1.0, 0.3, TRUE, FALSE);
	shapes.push_back(s);  */

	Shape *s = new RectangularPrism(0, 0.32, 0, 3.74, 0.74, 1.41, -0.0, 0.5, 0.7, 0.0);
	shapes.push_back(s);

	s = new TrapezoidalPrism(0, 1.06, 0, 1.62, 0.51, 1.41, 1.09, 0.55, -0.0, 0.7, 0.0, 0.5);
	shapes.push_back(s);

	s = new Cylinder(0.64, 0, 0.705, 0.32, 0.19, 0.0, 0.2, 0.2, 0.2, TRUE, TRUE);
	shapes.push_back(s);

	s = new Cylinder(0.64, 0, -0.705, 0.32, 0.19, 0.0, 0.2, 0.2, 0.2, TRUE, TRUE);
	shapes.push_back(s);

	s = new Cylinder(-0.52, 0, -0.705, 0.32, 0.19, 0.0, 0.2, 0.2, 0.2, TRUE, FALSE);
	shapes.push_back(s);

	s = new Cylinder(-0.52, 0, 0.705, 0.32, 0.19, 0.0, 0.2, 0.2, 0.2, TRUE, FALSE);
	shapes.push_back(s);

	s = new TriangularPrism(-1.72, 1.06, 0.0, 0.3, 0.15, 90.0, 1.41, -0, 0.9, 0.5, 0.0);
	shapes.push_back(s);
}
